<script setup lang="ts">
  ...
  const uniqueDates = computed(() => {
    const actDates = activities.value.map(a => a.date).filter(Boolean)
    const attrDates = attractions.value.map(a => a.date).filter(Boolean) 
    // 合併並去重
    return [...new Set([...actDates, ...attrDates])].sort()
  })
  ...
  function openAttractionForm(attr?: any) {
    if (attr) {
      ...
      attractionForm.value = {
        title: '', 
        date: selectedDate.value || startDateRaw.value,
        subtitle: '', 
        location_tag: '', 
        description: '', 
        highlights: '', 
...
<template>
  ...
  <div class="min-h-screen bg-[#FDFCF8] pb-24 font-sans text-stone-700">
    ...
    <div class="px-4 py-2 flex items-center">
        <button @click="goBack" class="flex items-center gap-1 text-[#606C38] font-bold m hover:text-[#283618] transition">
    ...
    <div class="banner rounded-2xl overflow-hidden mb-4 mx-4 shadow-lg shadow-stone-200 bg-gradient-to-r from-[#606C38] to-[#283618]">
      <div class="p-6 text-white">
        ...
        <p class="m opacity-90 mt-1 font-medium">{{ tripDates }} <span v-if="subtitleRaw">| {{ subtitleRaw }}</span></p>
      </div>
    </div>
    ...
    <div v-if="activeTab === 'itinerary' || activeTab === 'attractions'" class=" z-10 bg-[#FDFCF8]/95 backdrop-blur-sm border-b border-stone-200 pt-2 mb-4">
        <div class="flex overflow-x-auto px-4 pb-3 gap-3 no-scrollbar">
            <button @click="selectedDate = ''" class="px-4 py-2 rounded-full m font-bold whitespace-nowrap transition-all shadow-sm border" :class="selectedDate === '' ? 'bg-[#283618] text-white border-[#283618]' : 'bg-white text-stone-500 border-stone-200 hover:border-[#606C38]'">
                全部
            </button>
            
            <button v-for="date in uniqueDates" :key="date" @click="selectedDate = date" class="px-4 py-2 rounded-full m font-bold whitespace-nowrap transition-all shadow-sm border" :class="selectedDate === date ? 'bg-[#283618] text-white border-[#283618]' : 'bg-white text-stone-500 border-stone-200 hover:border-[#606C38]'">
                {{ date.slice(5) }}
            </button>
        </div>
    </div>
            ...
            <template v-else>
                <div v-for="act in filteredActivities" :key="act.id" 
                    ...
                    <div class="flex flex-col items-center w-14 border-r border-stone-100 pr-2">
                        <span class="text-[#BC4749] font-black font-mono m">{{ act.start_time || '--:--' }}</span>
                ...
                <div v-if="filteredActivities.length === 0" class="text-center py-20 text-stone-400">
                    <p>這一天還沒有行程喔</p>
                    <button v-if="isEditMode" @click="openActivityForm()" class="text-[#BC4749] test-s font-bold mt-2 hover:underline">+ 點此新增</button>
                </div>
            </template>
        </div>
        ...
        <div class="grid gap-6">
            <div v-for="attr in filteredAttractions" :key="attr.id" class="bg-white rounded-xl shadow-md overflow-hidden border border-stone-100 relative group hover:shadow-lg transition-all">
                ...
                <div class="bg-gradient-to-br from-[#D4A373] to-[#B08968] p-6 text-white pt-10 pb-8 relative">
                    ...
                    <p class="m text-[#FEFAE0]/90 font-medium">{{ attr.subtitle }}</p>
                </div>            

                <div class="p-5">
                    <p class="text-stone-600 m leading-relaxed mb-5 text-justify">{{ attr.description || '暫無描述' }}</p>
                    
                    <div class="bg-[#FEFAE0] rounded-lg p-4 mb-5 border border-[#E9EDC9]" v-if="attr.highlights || attr.must_eat">
                        <h4 class="m font-bold text-[#A98467] mb-2 flex items-center gap-1"><span>✨</span> 必訪亮點 </h4>
                        <ul class="space-y-2">
                            <li v-for="(line, idx) in parseLines(attr.highlights)" :key="'h'+idx" class="flex items-start gap-2 m text-stone-700">
                                <span class="mt-0.5 test-s text-[#A98467]">●</span><span>{{ line }}</span>
                            </li>
                            <li v-for="(line, idx) in parseLines(attr.must_eat)" :key="'m'+idx" class="flex items-start gap-2 m text-stone-700">
                                <span class="mt-0.5 test-s text-[#A98467]">🍽️</span><span>{{ line }}</span>
                            </li>
                        </ul>
                    </div>

                    <div class="grid grid-cols-1 gap-y-3 m text-stone-600 mb-6">
            ...
            <div v-if="filteredAttractions.length === 0" class="text-center py-12 border-2 border-dashed border-stone-200 rounded-xl bg-stone-50">
                <p class="text-stone-400 mb-2">{{ isEditMode ? '這一天還沒有安排景點' : '暫無景點介紹' }}</p>
                <button v-if="isEditMode" @click="openAttractionForm()" class="text-[#BC4749] font-bold hover:underline">新增第一個景點</button>
            </div>
    ...
    <section v-show="activeTab==='accommodation'" class="px-4 pb-20">
        <div class="flex justify-between items-center mb-4 pl-1">
            <h2 class="text-xl font-bold text-[#BC4749] flex items-center gap-2">
             ...
             <span class="test-s text-stone-400" v-if="accommodations.length > 0">共 {{ accommodations.length }} 間</span>
        </div>
        ...
        <div v-else class="space-y-8">
            <div v-for="item in accommodations" :key="item.id">
                <div class="bg-white rounded-xl shadow-md border border-stone-100 overflow-hidden relative group hover:shadow-lg transition-shadow">
                    ...
                    <div class="bg-[#6F4E37] text-white p-4">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-white/80 m mt-1 flex items-center gap-2">
                    ...
                    <div class="p-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8 mb-6 m">
                            <div class="flex gap-3">
                                <div class="text-[#BC4749] mt-0.5 text-lg">📍</div>
                                <div class="flex-1">
                                    <div class="text-stone-400 test-s mb-0.5">地址</div>
                            ...
                            <div class="flex gap-3">
                                ...
                                <div class="flex-1">
                                    <div class="text-stone-400 test-s mb-0.5">最近車站</div>
                            ...
                            <div class="flex gap-3">
                                ...
                                <div class="flex-1">
                                    <div class="text-stone-400 test-s mb-0.5">Check-in</div>
                            ...
                            <div class="flex gap-3">
                                <div class="text-stone-500 mt-0.5 text-lg">🕒</div>
                                <div class="flex-1">
                                    <div class="text-stone-400 test-s mb-0.5">Check-out</div>
                        ...
                        <div class="bg-[#F5F5F4] rounded-xl border border-stone-200 overflow-hidden shadow-sm mb-6" v-if="item.transportation && item.transportation.length > 0">
                            <div class="p-3 border-b border-stone-200 flex items-center gap-2 bg-[#E7E5E4]/50"><span class="text-xl">🚄</span>
                                <h3 class="font-bold text-[#6F4E37] m">交通方式</h3>
                            </div>
                            <div class="p-5">
                                <ol class="relative border-l-2 border-[#D4A373] ml-2 space-y-6">
                                    <li v-for="(trans, index) in item.transportation" :key="index" class="ml-6"><span class="absolute -left-[9px] flex items-center justify-center w-5 h-5 bg-stone-100 rounded-full border-2 border-[#D4A373] text-[10px] font-bold text-[#6F4E37] bg-white">{{ trans.step }}</span>
                                        <p class="m text-[#6F4E37] leading-relaxed font-medium">{{ trans.text }}</p>
                                    </li>
                                </ol>
                                <div class="mt-6 flex items-start gap-2 bg-white/60 p-3 rounded-lg border border-white/50" v-if="item.transport_note"><span class="text-[#D4A373] mt-0.5">💡</span>
                                    <p class="test-s text-stone-600 font-medium">{{ item.transport_note }}</p>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-gradient-to-r from-[#6B905C] to-[#463F3A] text-white p-4">
                    <div class="flex items-start gap-3">
                        <div class="text-2xl mt-0.5 text-[#FFE8D6]">✈️</div> <div>
                            ...
                            <div class="text-[#F0EFEB] test-s mt-1 flex gap-2 opacity-90 font-medium">
                ...
                <div class="p-5">
                    <div class="relative border-l-2 border-[#BC4749]/30 ml-2 space-y-8 my-2">
                        <div v-for="(step, idx) in trans.steps" :key="idx" class="ml-6 relative">
                            ...
                            <h4 class="font-bold text-[#283618] m mb-1">{{ Number(idx) + 1 }}. {{ step.title }}</h4>
                            <p class="test-s text-stone-500 leading-relaxed">{{ step.desc }}</p>

                            <div v-if="step.tip" class="mt-2 bg-[#FEFAE0] border-l-4 border-[#D4A373] p-2 text-[12px] text-[#606C38] rounded-r">
                    ...
                    <div class="bg-stone-50 p-3 rounded-lg flex justify-between items-center mt-6 mb-3 border border-stone-100">
                        <span class="test-s text-stone-400">預估票價</span>
        ...
        <div class="bg-[#FDFCF8] w-full max-w-md rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <div class="flex gap-2">
                    <div class="w-1/3">
                        <label class="test-s text-stone-500 mb-1 block">地區標籤</label>
                    ...
                    <div class="flex-1">
                        <label class="test-s text-stone-500 mb-1 block">景點名稱</label>
                ...
                <div>
                    <label class="test-s text-stone-500 mb-1 block">副標題</label>
                ...
                <div><label class="test-s text-stone-500 mb-1 block">介紹描述</label><textarea v-model="attractionForm.description" class="w-full border border-stone-300 p-2 rounded-lg bg-white h-24 m focus:outline-none focus:border-[#606C38]"></textarea></div>
                <div class="bg-[#FEFAE0] p-3 rounded-lg border border-[#E9EDC9]">
                    <label class="test-s font-bold text-[#D4A373] mb-1 block">✨ 必訪亮點 (一行一個)</label><textarea v-model="attractionForm.highlights" class="w-full border border-stone-200 p-2 rounded-lg bg-white h-20 m focus:outline-none focus:border-[#D4A373]"></textarea>
                    <label class="test-s font-bold text-[#D4A373] mt-2 mb-1 block">🍽️ 必吃美食 (一行一個，可選)</label><textarea v-model="attractionForm.must_eat" class="w-full border border-stone-200 p-2 rounded-lg bg-white h-16 m focus:outline-none focus:border-[#D4A373]"></textarea>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div><label class="test-s text-stone-500 mb-1 block">🚇 交通方式</label><input v-model="attractionForm.transport_info" class="w-full border border-stone-300 p-2 rounded-lg bg-white m focus:outline-none focus:border-[#606C38]" /></div>
                    <div><label class="test-s text-stone-500 mb-1 block">🎫 門票/費用</label><input v-model="attractionForm.ticket_price" class="w-full border border-stone-300 p-2 rounded-lg bg-white m focus:outline-none focus:border-[#606C38]" /></div>
                    <div class="col-span-2"><label class="test-s text-stone-500 mb-1 block">⏰ 營業時間</label><input v-model="attractionForm.opening_hours" class="w-full border border-stone-300 p-2 rounded-lg bg-white m focus:outline-none focus:border-[#606C38]" /></div>
                </div>
                <div><label class="test-s text-stone-500 mb-1 block">📍 Google Map URL</label><input v-model="attractionForm.map_url" class="w-full border border-stone-300 p-2 rounded-lg bg-white text-[#606C38] test-s focus:outline-none focus:border-[#606C38]" /></div>
                <div class="flex gap-3 mt-6 pt-2 border-t border-stone-100">
                    <button v-if="isEditingAttraction" @click="handleDeleteAttraction" class="bg-red-50 text-[#BC4749] px-4 py-3 rounded-xl font-bold m">刪除</button>
    ...
    <div v-if="showAccForm" class="fixed inset-0 bg-[#283618]/60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm" @click.self="showAccForm = false">
        <div class="bg-[#FDFCF8] w-full max-w-lg rounded-t-2xl sm:rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div class="space-y-4">
                ...
                <div><label class="test-s text-stone-500">Google Map URL</label><input v-model="accForm.google_map_url" class="w-full border border-stone-300 bg-white rounded px-2 py-2 text-[#606C38] focus:outline-none focus:border-[#606C38]" /></div>
                <div class="bg-stone-50 p-3 rounded border border-stone-200">
                    <label class="test-s font-bold mb-2 block text-stone-700">交通步驟</label>
                    <div v-for="(step, idx) in accForm.transportation" :key="idx" class="flex gap-2 mb-2"><span class="w-6 h-6 flex items-center justify-center bg-white border border-stone-300 rounded-full test-s mt-2 text-[#606C38] font-bold">{{ Number(idx)+1 }}</span><textarea v-model="step.text" class="flex-1 bg-white border border-stone-300 rounded px-2 py-1 h-16 resize-none focus:outline-none focus:border-[#606C38]"></textarea><button @click="removeTransportStep(idx)" class="text-[#BC4749] self-center">×</button></div>
                    <button @click="addTransportStep" class="w-full py-2 border border-dashed border-stone-300 rounded test-s text-stone-500 hover:bg-stone-100">+ 步驟</button>
                </div>
                
                <div class="flex gap-3 mt-6 pt-2 border-t border-stone-100">
                    <button v-if="isEditingAcc" @click="handleDeleteAccommodation" class="bg-red-50 text-[#BC4749] px-4 py-3 rounded-xl font-bold m">刪除</button>
    ...
    <div v-if="showTransportForm" class="fixed inset-0 bg-[#283618]/60 z-50 flex items-center justify-center p-4" @click.self="showTransportForm = false">
        <div class="bg-[#FDFCF8] w-full max-w-lg rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div class="space-y-4">
                 <div class="flex gap-2">
                    <div class="flex-1"><label class="test-s text-stone-500 mb-1 block">路線名稱</label><input v-model="transportForm.title" placeholder="例: 關西機場 → 難波" class="w-full border border-stone-300 p-2 rounded-lg bg-white font-bold focus:outline-none focus:border-[#606C38] focus:ring-1 focus:ring-[#606C38]" /></div>
                    <div class="w-1/3"><label class="test-s text-stone-500 mb-1 block">交通工具</label><input v-model="transportForm.transport_type" placeholder="例: 南海電鐵" class="w-full border border-stone-300 p-2 rounded-lg bg-white focus:outline-none focus:border-[#606C38]" /></div>
                 </div>
                 <div class="flex gap-2">
                    <div class="flex-1"><label class="test-s text-stone-500 mb-1 block">行車時間</label><input v-model="transportForm.duration" placeholder="例: 約48分鐘" class="w-full border border-stone-300 p-2 rounded-lg bg-white focus:outline-none focus:border-[#606C38]" /></div>
                    <div class="flex-1"><label class="test-s text-stone-500 mb-1 block">預估票價</label><input v-model="transportForm.price" placeholder="例: ¥970 / 人" class="w-full border border-stone-300 p-2 rounded-lg bg-white font-bold text-[#BC4749] focus:outline-none focus:border-[#606C38]" /></div>
                 </div>
                 
                 <div class="bg-stone-50 p-3 rounded-lg border border-stone-200">
                    <label class="test-s font-bold text-stone-700 mb-2 block">路線步驟</label>
                    <div v-for="(step, idx) in transportForm.steps" :key="idx" class="mb-4 pb-4 border-b border-stone-200 last:border-0 last:pb-0 last:mb-0">
                        <div class="flex justify-between items-center mb-1">
                             <span class="test-s bg-[#E9EDC9] px-2 py-0.5 rounded text-[#283618] font-bold">Step {{ Number(idx)+1 }}</span>
                             <button @click="removeTransStep(idx)" class="text-[#BC4749] test-s" v-if="transportForm.steps.length > 1">刪除</button>
                        </div>
                        <input v-model="step.title" placeholder="步驟標題" class="w-full border border-stone-300 p-2 rounded mb-2 m font-bold focus:outline-none focus:border-[#606C38]" />
                        <textarea v-model="step.desc" placeholder="詳細說明..." class="w-full border border-stone-300 p-2 rounded mb-2 test-s h-16 focus:outline-none focus:border-[#606C38]"></textarea>
                        <input v-model="step.tip" placeholder="💡 黃色底提示 (選填)" class="w-full border border-[#D4A373]/30 bg-[#FEFAE0] p-2 rounded test-s text-[#6F4E37] focus:outline-none focus:border-[#D4A373]" />
                    </div>
                    <button @click="addTransStep" class="w-full py-2 border border-dashed border-stone-300 rounded text-stone-500 test-s mt-2 hover:bg-stone-100">+ 新增步驟</button>
                 </div>

                 <div><label class="test-s text-stone-500 mb-1 block">Google Map URL</label><input v-model="transportForm.map_url" placeholder="http..." class="w-full border border-stone-300 p-2 rounded-lg bg-white text-[#606C38] test-s focus:outline-none focus:border-[#606C38]" /></div>

                 <div class="flex gap-3 mt-6 pt-2 border-t border-stone-100">
                    <button v-if="isEditingTransport" @click="handleDeleteTransport" class="bg-red-50 text-[#BC4749] px-4 py-3 rounded-xl font-bold m">刪除</button>
